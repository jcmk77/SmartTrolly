...
3. start `smarttrolly-ap` when no saved client profile connects.
...
1. `smarttrolly-ap` is the predictable recovery network. Its default SSID
   is `SmartTrolly`, its Pi gateway is `10.42.0.1/24`, and NetworkManager
   shared mode supplies DHCP and DNS.
2. `smarttrolly-facility-*` is a customer/facility profile saved through
...
- returns a validated `smarttrolly://` handoff link for the Windows
...
```bash
mkdir -p /tmp/smarttrolly-ap-secret
chmod 0700 /tmp/smarttrolly-ap-secret
read -rsp "Choose SmartTrolly Wi-Fi password: " ROBOT_AP_PASSWORD
echo
printf '%s\n' "${ROBOT_AP_PASSWORD}" >/tmp/smarttrolly-ap-secret/psk
chmod 0600 /tmp/smarttrolly-ap-secret/psk
...
```bash
ROBOT_AP_PSK_FILE=/tmp/smarttrolly-ap-secret/psk \
  ./Trolly/scripts/generate_wifi_ap_config.sh \
  --output-dir /tmp/smarttrolly-ap

sudo install -o root -g root -m 0600 \
  /tmp/smarttrolly-ap/smarttrolly-ap.nmconnection \
  /etc/NetworkManager/system-connections/
sudo nmcli connection reload

rm -f /tmp/smarttrolly-ap-secret/psk
rm -f /tmp/smarttrolly-ap/smarttrolly-ap.nmconnection
```

Install the Wi-Fi startup gate and provisioning service without starting them:

```bash
./Trolly/scripts/install_wifi_provisioning_service.sh --dry-run
./Trolly/scripts/install_wifi_provisioning_service.sh
```
...
nmcli -f connection.id,connection.autoconnect,connection.autoconnect-priority,802-11-wireless.ssid,802-11-wireless.mode,802-11-wireless.band,802-11-wireless.channel,ipv4.method,ipv4.addresses \
  connection show smarttrolly-ap
```
...
```bash
./Trolly/scripts/install_wifi_provisioning_service.sh --enable
```

When deploying while SSH is connected through `SmartTrolly`, do not restart
the network gate inside that SSH session. Install and enable the units for the
next boot instead:

```bash
./Trolly/scripts/install_wifi_provisioning_service.sh --enable-next-boot
```
...
confirmed facility Wi-Fi or `smarttrolly-ap`, while rejecting source
addresses outside the active `wlan0` IPv4 subnet. The AP-specific computer
configuration action still requires `smarttrolly-ap` and `10.42.0.0/24`.
...
sudo nmcli connection modify smarttrolly-ap \
  connection.autoconnect yes \
  connection.autoconnect-priority 50
sudo nmcli connection up smarttrolly-ap
sudo systemctl start my-bot-robot.service
```

Join `SmartTrolly` from the operator computer, verify `ping 10.42.0.1`, then open:
...
10. On Windows, optionally select **Configure SmartTrolly on Windows** and
    approve UAC. On Linux, use the displayed ROS domain and verify discovery
    directly; no Windows handoff is required.
11. Run SmartTrolly Diagnostics and verify the Pi topics and UI.
...
to `SmartTrolly`, and correct the facility settings.
...
```bash
sudo systemctl stop my-bot-robot.service
sudo nmcli connection modify smarttrolly-ap connection.autoconnect yes
sudo nmcli connection up smarttrolly-ap
sudo systemctl start my-bot-robot.service
```
...
```bash
nmcli -t -f NAME connection show | grep '^smarttrolly-facility-'
...
that existed before SmartTrolly was installed. Removal targets the selected
